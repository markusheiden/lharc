...
static FILE *file1, *file3;      /* neu: vorher extern */
static ulong origsize, codesize; /* neu: vorher extern */

static _DTA _dta;
static int file2, args, c_err, skipped, cmd, cmdupdate, cmdlist, oldtos;
static int _gemdos=SUCCS, Nfile, fbfiles, found, sfx;
static int errorlevel, bad_tab;
static long arcpos0, arcpos1, lastarcpos, lastarclen, nextarcpos, nextoffset, arclen, maxlen, old_afx;
static long o_handle=-1, i_handle=-1, o_dev=-1, i_dev=-1;
...
static uchar *bbuf, *ebuf, *fbuf, *tbuf, *fbnxt, *o_dir;
static ulong fblft, fbmax;
static filebuf *fblast;

uchar *print_ptr;      /* NEU */
uchar print_buf[1536]; /* NEU */

static int columns, lines;
static uchar *env_workdir, *env_basedir, *env_unpacked, *env_switches;
...
static uchar compress;                            /* auch in hufst.s */
static uchar method=5;                            /* auch in hufst.s */
long bsize;                                       /* auch in hufst.s */
static OutRec outrec;                             /* neu: vorher extern */
static int outfile;                               /* auch in hufst.s */
uchar obj;                                        /* auch in test_afx.s */
static uchar has_crc;                             /* auch in dec*.s */
static uchar *infname;                            /* auch in util.s */
static int crc;

int *lson, *rson;                       /* Pufferzeiger fÅr LHarc 1.13/Larc */
#ifndef SFX
int *dad;
#endif

uchar *text;                            /* Pufferzeiger fÅr LH5 */ /* childcount wird nicht benutzt? */
uint *level, *childcount;
short *position, *parent, *prev, *next;

uchar *textbuf;                         /* Pufferzeiger fÅr alle Codier-/Decodierroutinen */

static int hdr_len, header_len, ignfile;
static uchar *pager, *com_name;
static uchar base, star='*', pnt='.';
...
static uchar flg_l, flg_o, flg_5, flg_U=1, flg_w,   flg_chk, flg_r,
             ...
             flg_F, flg_O;
static uchar flg_q; /* -> decode5.s */
       uchar flg_n; /* -> util.s */
static uchar flg_Y; /* -> hufst.s */
...
uchar *buffer_gen; /* Filepuffer fÅr test_afx */
uchar *buffer_3;   /* Output-Filepuffer fÅr Decode0/1 etc. */

static uchar backup2[MAXPATH], pathname[MAXPATH];
...
# define EXIT_TIMER timer=clock() - timer; sprintf(print_buf,"\r\n %s : %ld ms ", T_TIME, (timer*5)); Print(1)
#else
# define INIT_TIMER
# define EXIT_TIMER
#endif
...
{
  lseed = A * ((lseed % Q) + R) - R * (lseed / Q);

  if (lseed<0)
    lseed+=M;

  return((int) lseed & 4095);
}


/* File lîschen (Schreibschutz aufheben) */
...
/* Testen auf mehrere Files auf einmal */
...
/* Testen auf Wildcards im Filenamen */
...
/* String p ausgeben mit nl newlines */
...
/* Ausgabepuffer ausgeben mit nl newlines */
static void Print(int nl)
{
  print_ptr=print_buf; /* Pufferzeiger zurÅcksetzen */
  print(print_buf,nl); /* Puffer ausgeben */
}


/* Ausgabepuffer ausgeben (ggf. mit ext. Programm) */
...
/* Auf Taste warten, danach nl newlines */
...
/* Benutzungshinweise ausgeben */
static void show_usage(int stop)
{
  int i, line, max_lines=lines-2;
  char key;
  ...
  for(i=0,line=(stop ? 3 : 1);usage[i];i++,line++)
  {
    print(usage[i],1);
    ...
    if(line==max_lines)
    {
      ...
      print("\r\033l\r",0); /* Zeile lîschen (ESC l) */
...
/* path mit \ abschlieûen (set=1) oder nicht (set=0) und Test auf Zustand */
...
static void lha_exit(void)
{
  ...
  if (flg_K) /* Akustisches Signal ausgeben */
  {
...
/* Fehlerbehandlung fÅr Codec */
void ReadErrorEnc (void)
{
  error (RDERR, infname, SUCCS);
}

void WriteErrorEnc (void)
{
  error (WTERR, backup2, SUCCS);
}

void ReadErrorDec (void)
{
  error (RDERR, NULL, SUCCS); /* Ñndern ??? */
}

void WriteErrorDec (void)
{
  error (WTERR, NULL, SUCCS);
}


/* allgemeine Fehlerroutine err=SUCCS: Programmabbruch */
  ...
  if (errcode>=0)
  {
    ...
    if (shell)
    {
      if (execute && !copying)
      {
        ...
        if (errcode==WTERR)
          sprintf(print_buf," (%s?)",T_DISK_FULL);
...
/* Auf Tasten warten und ausgeben */
...
void sethdr(uchar *fn,uint attr,_DOSTIME *time,LzHead *h,int name)
{
  ...
  memset(h,0,sizeof(LzHead)); /* Header lîschen */
  ...
  if (file3)
  {
    h->OrgSiz=Fseek(0l,fileno(file3),SEEK_END);
  ...
  *ptr++=(uchar) (crc>>8);
  ...
  if (make)
  {
    if (outrec.cnt<=(len+1024) || (bsize>size && outrec.cnt<=size))
      ShipOut(&outrec, outfile);

    outrec.buffer_start=outrec.ptr;
    outrec.buffer_last=(outrec.ptr+=len);
    outrec.buffer_cnt=(outrec.cnt-=len);
    outrec.buffered=1;
  }
  ...
  if (outrec.buffered)
  {
    memcpy(outrec.buffer_start,ptr-8,7);
    memcpy(outrec.buffer_start+7,ptr,len-7);
  }
  else
  {
...
/* Wandlung von Dateinamen nach 8+3 */
...
/* Wandlung nach lower (flag=1) oder upper (flag=0) */
...
/* Test auf Enthaltensein (Wildcard-matching) */
  ...
  *comment='\0';
  ...
  else if (lastarcpos<(arclen-16))
  {
    if (search_lzh(arc,-1)>0)
    {
      ...
      #endif
        print(M_GARBAGE,1);   /* hier fehlt ein Space vor M_GARBAGE */
    ...
    {
      sprintf(print_buf, " %s (%ld %s)", T_GARBAGE, arclen-lastarcpos, T_BYTES);
...
/* Speicheranforderung fÅr Filepuffer */
...
/* Attribute (Zeit, Datum, LÑnge, Dateiattribute) einer Datei auslesen */
...
void mklist(void)
{
  ...
  #endif
    {
      sprintf(print_buf,"\r %s : (%d/%d), %s : %4d", T_PATTERN, i+1, patno, T_FILE_MATCH, Nfile);
      Print(0);
    }
  ...
  if (shell)
  {
    ...
    {
      stop=SUCCS;
      sprintf(print_buf,M_FT_OVERFLOW,ignfile);
      ErrorAlert(print_buf);
      error(-1,NULL,SUCCS);
    }
  }
  ...
  {
    if (ignfile)
     sprintf(print_buf,"\b\b\b\b%4d\r\n%s, %d %s", T_FT_OVERF, ignfile, T_FILES_IGN);
    else
...
static void MakeBuffers(void)
{
  ...
  if (tbuf==NULL)
  {
    if ((buf=(long) Malloc(TEXTBUF+128))!=0L)
    {
      tbuf=(uchar *) buf;
      buf=(buf+=63) & (~15L);

      /* memset(tbuf,0,TEXTBUF+128) */
...
static void copyfile(FILE *Source,FILE *Dest,uchar *outfname,long size,ProcInd_t *ProcInd,int bufflg)
{
  register uchar *ptr;
  register long n, block;

  crc_t   *crc_o   = NULL;
  crypt_t *crypt_o = NULL;
  ...
  while (size>0)
  {
    ...
    if (crc_o)
    {
      block_crc(ptr, n, crc_o);
    }

    if (crypt_o)
    {
      encrypt(ptr,n, crypt_o);
    }

    if (ProcInd)
    {
      do_ProcInd(ProcInd, n);
    }
...
void freeze(uchar *p, int attr, int file, int Case)
{
  ...
  ProcInd_t *ProcInd=create_ProcInd (flg_n, star, pnt);
  ...
  if (file>0)   /* Filenummer ausgeben */
  {
    sprintf(print_buf,"(%d/%d): ",file,Nfile);
    print(print_buf,0);
  }

  if (attr & FA_DIR)   /* ggf. Directory ausgeben */
    sprintf(print_buf,"%s: %s",p,T_DIRECTORY);
  else
    sprintf(print_buf,"%s",p);

  print(print_buf,1);


  if (flg_e)   /* Kommentar einlesen */
  {
    get_comment(stdin);
  }


  infname=p;   /* File fÅr ggf. Fehlerausgabe */

  if (attr & FA_DIR)   /* Directory einpacken */
  {
    Hdr2.OrgSiz=0;
    Hdr2.PacSiz=0;
    ...
    start_ProcInd(ProcInd, T_STORING, 1L, 1L, maxblk);
    do_ProcInd(ProcInd, 1L);
    end_ProcInd(ProcInd, T_STORED, 0L, 0L, 100);
  }
  else                 /* Files einpacken */
  {
    if (!unpck || cmd!='C')   /* Make Header */
    {
      wthdr(&Hdr2,SUCCS,unpck);
    }

    dummy_file._file = outfile;   /* FÅr comp_file.arc */

    comp_file.arc        = &dummy_file;
    comp_file.file       = file3;
    comp_file.outrec     = &outrec;
    comp_file.size       = Hdr2.OrgSiz;
    comp_file.compsize   = 0;
    comp_file.crc        = create_crc();
    comp_file.crypt      = (flg_Y) ? create_crypt(-1) : NULL;

    if (!unpck)   /* Encode */
    {
      start_ProcInd(ProcInd, T_FREEZING, comp_file.size, N*2, maxblk);

      switch (method)
      {
        case 0: EncodeOld(&comp_file, ProcInd, compress); break;
        case 1: Encode1  (&comp_file, ProcInd, compress); break;
        case 5: Encode5  (&comp_file, ProcInd, compress); break;
      }

      end_ProcInd(ProcInd, T_FREEZED, comp_file.size, comp_file.compsize, ratio(comp_file.compsize, comp_file.size)/10);
    }

    origsize = comp_file.size;
    codesize = comp_file.compsize;

    crc      = get_crc(comp_file.crc);

    destroy_crc (comp_file.crc);
    destroy_crypt (comp_file.crypt);


    if (cmd=='C' && ((origsize+min_len-1)/min_len)<=((codesize+header_len+min_len-1)/min_len))   /* AFX bei schlechter Komprimierung */
    {
      flg_unpacked=1;

      Hdr2.PacSiz=origsize;

      start_ProcInd(ProcInd, T_STORING, origsize, N*2, maxblk);
      do_ProcInd(ProcInd, origsize);
      end_ProcInd(ProcInd, T_STORED, origsize, origsize, 100);
    }
    else if (unpck || (!compress && codesize>=origsize))   /* Store */
    {
      flg_unpacked=1;

      Hdr2.PacSiz=origsize;
      arcpos=arcpos1+origsize;
      ...
      if (!unpck)
      {
        if (outrec.buffered)
        {
          outrec.ptr=outrec.buffer_last;
          outrec.cnt=outrec.buffer_cnt;
        }
        else
        {
          OpenOut(&outrec,buffer_gen,bsize);
      ...
      start_ProcInd(ProcInd, T_STORING, origsize, N*2, maxblk);
      copyfile(file3,NULL,backup2,origsize,ProcInd,1);   /* ProcInd intern */
      end_ProcInd(ProcInd, T_STORED, origsize, origsize, 100);
    }
    else   /* Encoded */
    {
    ...
    if (!((cmd=='C') && (flg_unpacked==1)))   /* Header schreiben */
    {
...
/* Kommentar einlesen und archivieren */
  ...
  for (;;)
  {
    ...
    else
    {
      if ((old != file) && (file <= Nfile))
      {
        ...
        {
          if (found)
            skip (infname, (flg_A) ? M_F_EXISTS : M_NEW_O_SAME, 0);
        }
        else
      #endif
        {
...
#ifdef __SHELL__
  if (shell)
    SetMsg((flg_d) ? M_DELETE_F : M_DELETE_A);
  else
#endif
  print((flg_d) ? M_DELETE_F : M_DELETE_A, 1);
...
int search_lzh(FILE *input, int mode)
{
  ...
  if ((len=Fread(fileno(input),1024l*10,textbuf))>5)
  {
    register uchar *ptr=textbuf, *last=ptr+len-5, c, s='-';
      ...
      if (*ptr==s && ptr[4]==s)
      {
        ...
        if (c=='l' || c=='L' || c=='a' || c=='A')
        {
          if (ptr>=(textbuf+2))
          {
            ...
            fseek(input,nextarcpos=pos+((ptr-2)-textbuf),SEEK_SET);
            return(1);
          }
        }
...
void append(void)
{
  ...
  if (file1)
    message(M_UPDATE,arcname);
  else if (Device)
    message(M_FREEZE,arcname);
  else
    message(M_CREATE,arcname);
...
static void skip(uchar *fname, uchar *msg, int plus)
{
  ...
  if (shell)
  {
    ...
    sprintf(print_buf,"%s %s: %s", T_SKIPPED, get_fname(name), msg);
    DialIndicator(1);
    if (idx)
      SetMsg(print_buf);
  }
...
#endif
  {
    sprintf(print_buf,"%s %s: %s", T_SKIPPED, fname, msg);
    Print(1);
    skipped+=plus;
  }
}
...
Again:
  if (flag || flg_x)
  {
    ...
    if (flag>=0)
      yn=(flg_m>0 && flg_m<3) ? KEY_YES : KEY_NO;
    else
      yn=KEY_YES;
      ...
      *p='\0';
      ...
      if (attr<0)
      {
        if (yn==KEY_NO)
        {
          ...
          if ((yn=get_key(KEYS_CHOOSE))==KEY_NO)
            return(FAULT);
          else if (yn==KEY_ALL)
          {
            flg_m=(flg_m) ? 1 : 2;
  ...
  if (!flag)
  {
    if ((attr=Attrib(name))>=0)
    {
      ...
      else if ((flg_m & 0x01)==0)
      {
        ...
        if ((yn=get_key(KEYS_CHOOSE2))==KEY_RENAME)
        {
          print(M_ENTERNEW,0);
          gets(path);
...
int extract_file (void)
{
  ...
  if ((m=tstID(Hdr1.HeadID))<0)                                         /* Unbekannte Packmethode */
  {
    skip(filename,M_UNKNOWN_M,1); return (FAULT);
  }
  else if (cmd=='E' && maxlen>0 && Hdr1.OrgSiz>maxlen)                  /* File zu groû */
  {
    skip(filename,M_FILE_LONG,1); return (FAULT);
  }
  else if (cmd=='E' && !flg_a && (Hdr1.Attr & (FA_HIDDEN|FA_SYSTEM)))   /* Hidden/System-File */
  {
    skip(filename,M_HIDDEN_F,1); return (FAULT);
  }
  else if (Hdr1.crypted && !flg_Y)                                      /* File crypted */
  {
    skip(filename,M_CRYPT_F,1); return (FAULT);
  }
  else if (Hdr1.Attr & FA_DIR)                                          /* Directory */
  {
    if (cmd=='E' && flg_x>0 && (succs=tstdir(strcat(pathname,"\\"),1))!=0)
    {
      if (UnixFile)
        sprintf(print_buf, "%s (%s): %s", pathname, filename, T_DIRECTORY);
      else
        sprintf(print_buf, "%s: %s", pathname, T_DIRECTORY);

      print(print_buf,1);

      if (cmd!='T' && comment && *comment && flg_x && (cmd!='P' || pager))
      {
        print(comment,1);
      }

      start_ProcInd(ProcInd, T_MELTING, 1L, 1L, maxblk);
      do_ProcInd(ProcInd, 1L);
      end_ProcInd(ProcInd, T_MELTED, 0L, 0L, 0);

      if (flg_d && cmd!='P' && succs)
        message(T_DELETING,filename);
    }
    else
      skip(filename,T_DIRECTORY,0);
  }
  else if (cmd!='E' || tstdir(pathname,0))                              /* Files */
  {
    origsize=Hdr1.OrgSiz;
    ...
    switch(cmd)
    {
      ...
      case 'T': sprintf(print_buf,"%s",filename);
                print(print_buf,1);

                file3=NULL;
                break;

      case 'P': sprintf(print_buf,"<<< %s >>>\r\n\r\n",filename);
                pager_print(0);

                sprintf(print_buf,"%s",(pager) ? filename : NULL);
                print(print_buf, 1);
    ...
    decomp_file.arc        = file1;
    decomp_file.file       = file3;
    decomp_file.outrec     = NULL;
    decomp_file.size       = Hdr1.OrgSiz;
    decomp_file.compsize   = Hdr1.PacSiz;
    decomp_file.crc        = has_crc ? create_crc() : NULL;
    decomp_file.crypt      = (flg_Y && Hdr1.crypted) ? create_crypt(Hdr1.PacSiz) : NULL;

    start_ProcInd(ProcInd,cmd=='T' ? T_TESTING : T_MELTING, decomp_file.size, N*2, maxblk);

    switch(m)
    {
      case 1:       DecodeOld(&decomp_file, ProcInd); break;
      case 3:       Decode1  (&decomp_file, ProcInd); break;
      case 4: succs=Decode5  (&decomp_file, ProcInd); break;
      default: copyfile(file1,file3,pathname,decomp_file.size,ProcInd,0);
    }

    end_ProcInd(ProcInd,cmd=='T' ? T_TESTED : T_MELTED, 0L, 0L, 0);

    crc = get_crc(decomp_file.crc);

    destroy_crc (decomp_file.crc);
    destroy_crypt (decomp_file.crypt);
    ...
    if (!succs)
    {
      ...
      sprintf(print_buf, "\r %s ", M_BADTAB);
      Print(1);
      ...
      if (shell)
      {
        ...
        sprintf(msg, "%s: %s",get_fname(filename), T_BAD_TBL);
        SetMsg(msg);
      }
      #endif
    }
    else if (has_crc && Hdr1.crc!=crc)
    {
      errorlevel|=2;
      c_err++;
      ...
      sprintf(print_buf, "\r %s ", T_CRC_ERR);
      Print(1);
      ...
      if (shell)
      {
        ...
        sprintf(msg, "%s: %s", get_fname(filename), T_CRC_ERR);
        SetMsg(msg);
      }
    #endif
    }
...
int extract(void)
{
  ...
  if (flg_d)   /* Flag: bearbeitete Dateien lîschen */
  {
    flg_t = 0;
  #ifdef __SHELL__
    if (!shell)
  ...
  while (gethdr(file1,&Hdr1))   /* Hauptschleife */
  {
    succs=FAULT;
  ...
  if (bad_tab)
  {
    sprintf(print_buf,"%s :%5d", T_BAD_TBLS, bad_tab);
    Print(1);
  }

  if (c_err)
  {
    sprintf(print_buf,"%s :%5d", T_CRC_ERRS, c_err);
    Print(1);
  }

  if (skipped)
  {
    sprintf(print_buf,"%s :%5d", T_SKIPPED_F, skipped);
    Print(1);
  }

  if (garbage)
  {
    sprintf(print_buf,"%s :%5d", T_CRASHED_F, garbage);
    Print(1);
  }
...
static void make_id(uchar* ID, uchar* return_id)
{
  strncpy(return_id,ID,5);
  return_id[5]='\0';
}


static void make_attr(int attr, uchar* return_attr)
{
  const uchar attr1[8]=" shr-da";
  const uchar attr0[5]=" --w";
  register int i, j;

  for (i=1,j=1;i<4;i++,j<<=1)
  {
    if (attr & j)
    {
      return_attr[i]=attr1[i];
    }
    else
    {
      return_attr[i]=attr0[i];
    }
  }
...
int list(void)
{
  register uchar *p, attr[5], id[6], name[15];
  ...
  static const uchar *list_format="%-14s %8lu %8lu %3d.%1d%% %2d-%02d-%02d %2d:%02d:%02d %-4s %-5s %04X  [%d]\r\n";
  ...
  if (flg_x<3) /* Langes Format */
  {
    sprintf(print_buf,"\r\n %-13s %-8s %-8s %-6s %-8s %-8s %-4s %-5s %-4s %s\r\n-------------- -------- -------- ------ -------- -------- ---- ----- ---- -----\r\n",
            T_NAME, T_ORIGINAL, T_PACKED, T_RATIO, T_DATE, T_TIME, T_ATTR, T_TYP, T_CRC, T_LEVEL);
  }
  else /* Kurzes Format */
  {
    sprintf(print_buf,"\r\n %s\r\n--------------\r\n", T_NAME);
  ...
  while (gethdr(file1,&Hdr1))
  {
    if (matchpat(matchfilename,-1,Hdr1.Attr))
    {
      if (flg_x<3) /* AusfÅhrliches Format */
      {
        rt=ratio(Hdr1.PacSiz,Hdr1.OrgSiz);
        make_attr(Hdr1.Attr, attr);
        make_id(Hdr1.HeadID, id);

        if (flg_x) /* Langes Format */
        {
          if (Hdr1.crypted)
          {
            print_ptr+=sprintf(print_ptr,"%s (%s)\r\n", filename, T_ENCRYPTED);
          }
          else
          {
            print_ptr+=sprintf(print_ptr,"%s\r\n", filename);
          }
          ...
          sprintf(print_ptr,list_format,sysid(SystemId),Hdr1.OrgSiz,Hdr1.PacSiz,rt/10,rt%10,(tim->year+80)%100,tim->mon,
                  tim->day,tim->hour,tim->min,tim->sec*2,attr,id,Hdr1.crc,Hdr1.Level);
          pager_print(0);
        }
        else /* kurzes Format */
        {
          slash(filename,0); /* Tieferliegende Dateien mit '+' kennzeichnen */
          ...
          if (strlen(p)>13) /* öberlange Namen abschneiden */
          {
            name[13]='>';
            name[14]='\0';
          }

          sprintf(print_ptr,list_format,name,Hdr1.OrgSiz,Hdr1.PacSiz,rt/10,rt%10,(tim->year+80)%100,tim->mon,
                  tim->day,tim->hour,tim->min,tim->sec*2,attr,id,Hdr1.crc,Hdr1.Level);
          pager_print(0);
        }
  ...
  if (Fno || Dno)
  {
    if (flg_x<3) /* Langes Format */
    {
      ...
      print_ptr+=sprintf(print_ptr,list_end);
      sprintf(print_ptr," %4d %7s, %8lu %8lu %3d.%1d%% %2d-%02d-%02d %2d:%02d:%02d\r\n %4d %s\r\n",
              Fno, T_FILES, Osize, Psize, rt / 10, rt % 10,(tim->year + 80) % 100, tim->mon,
              tim->day, tim->hour, tim->min, tim->sec * 2,Dno, T_DIRS);
    }
    else /* Kurzes Format */
    {
      sprintf(print_buf,"--------------\r\n  %3d %s,\r\n  %3d %s\r\n", Fno, T_FILES, Dno, T_DIRS);
    }
  }
  else
  {
...
void executecmd(void)
{
  ...
  INIT_TIMER;
  arcpos0=back_1=back_2=found=bad_tab=c_err=skipped=garbage=ignfile=outrec.buffered=0;
...
static void read_environment (void)
{
  ...
  columns = 80; /* Defaultwerte fÅr Ausgabe */
  lines   = 25;
  ...
  if ((env=getenv("TMP"))!=NULL || (env=getenv("TMPDIR"))!=NULL || (env=getenv("ARCTMP"))!=NULL)
    env_workdir=env;

  if ((env=getenv("LHARCDIR"))!=NULL)
    env_basedir=env;

  if ((env=getenv("UNPACKED"))!=NULL)
    env_unpacked=env;

  if ((env=getenv("LHARC"))!=NULL || (env=getenv("LHA"))!=NULL)
    env_switches=env;
}
...
static void init_lharc(void)
{
  ...
  lseed = (long) clock(); /* init temp-seed */
  ...
  table=upper; /* Konvertierungstabelle lower->upper erstellen */
  for (c=0;c<256;c++)
    if (c>='a' && c<='z')
      *table++=c^32;
    else if (c=='Å')
  ...
  getcwd(act_dir,MAXPATH); /* get actual dir */
  unix2dos(act_dir,0);
  slash(act_dir,1);
  fnamecnv(act_dir,-1);
}
...
#endif
{
  register uchar *p, *q, *env, *env_end, **old_argv=argv;

#ifdef __030__
  extern int get_cookie (long, long *);
  long cpu;
  if (!get_cookie ('_CPU', &cpu) || (cpu < 20L))
  {
    /* LHarc 030er-Version, aber eine CPU < 68020:
     * Fehlermeldung direkt Åber Cconws() ausgeben, um
     * mîglichst keinen 030er-Code zu verwenden ...
     */
    Cconws (title);
    Cconws (M_WRONG_CPU);
    return (64); /* "nicht implementiertes Kommando" - stimmt ja auch irgendwie :-} */
  }
#endif
...
#endif
  {
    ...
    *fileptr++='\0';
    ...
    if (env_workdir!=NULL)
    {
      flg_w++; /* Arbeitsverzeichnis festlegen */
      strcpy(workdir,env_workdir);
    }

    if (env_basedir!=NULL)
    {
      strcpy (basedir, env_basedir);
      slash  (basedir,1);
    }

    if (env_unpacked!=NULL)
    {
      flg_U++; /* Dateien unkomprimiert archivieren */
      unpack=env_unpacked;
    }

    if (env_switches!=NULL)
    {
      env=env_switches;

      for (env_end=env_switches;*env_end;env_end++)
        if (*env_end==' ' || *env_end=='\x08') /* TAB */
          *env_end=0;
